...
export class Game {
    constructor(canvas, turnIndicator, statusDiv, difficulty = 3, tigerRangeMultiplier = 1.0) {
        this.canvas = canvas;
    ...
    generateTerrain() {
        ...
        const minTerrainSeparation = tigerDiameter * 1.5;
        ...
        let attempts = 0;
        do {
            largeTerrain = generateSingleTerrain(Math.SQRT2);
            attempts++;
        } while (
            attempts < 30 && 
    ...
    // CORRECTED: Use Systems.distance
    isTerrainOverlapping(terrain1, terrain2, minSeparation) {
        if (!terrain1 || !terrain2) return false;
        const dist = Systems.distance(terrain1.center, terrain2.center);
    ...
    checkCompulsoryPounce() {
        ...
        this.visibleHuntersThisTurn = targets;
